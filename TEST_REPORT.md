...
| Every ten-frame has ten distinct slots; counters cannot duplicate a slot | Browser walk verifies ten unique `data-slot` controls in the count and bond frames; domain test checks unique slot toggles | Pass |
...
| 7+3 visibly completes ten and produces 7+3=10 | Browser walk fills the ten-frame, checks feedback, and captures `evidence/02-make-ten.png` | Pass |
| Removing four from nine produces five with no negative quantity | Seed-packet story starts at nine, removes four counters, then checks `9 − 4 = 5`; domain counter model clamps changes between zero and ten | Pass |
| Zero is explicit, not missing input | Learner sees “Show 0,” checks with all spaces empty, and receives explicit zero feedback before continuing | Pass |
| Repeated retries preserve counter state and progress | Browser walk checks the same incorrect set twice, confirms its one selected counter remains, adds the missing counters, and continues; a separate reload test resumes the saved state | Pass |
| Fresh final problem can be completed with optional hints and an explanation | Browser walk uses both hints, completes the new two-step duck story, and reads the displayed equation and result | Pass |
| Gentle feedback, retry, and success gating | Incorrect count shows a relevant retry cue and no next action; correct count reveals the next action only after Check | Pass |
...
The browser test was run with the static server started by the skill's `with_server.py` wrapper. An unauthenticated HTTPS readback returned HTTP 200 for the page, CSS, JavaScript module, screenshot, and walkthrough:

- Preview: <https://qiu-guanzong.github.io/number-garden-make-ten/>
- Public source and commit: <https://github.com/QIU-Guanzong/number-garden-make-ten/tree/main> (`e8f0aef`)
